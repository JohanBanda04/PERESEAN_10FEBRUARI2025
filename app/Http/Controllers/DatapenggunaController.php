...
class DatapenggunaController extends Controller
{
    ...
    {
        //cara penggunaan gate pada controller
        //$this->authorize('admin');
        //$query = Satker::query();
        $query_user = UserModel::query();

        if (!empty($request->nama_pereseanuser_cari)) {
            $query_user->where('username', 'like', '%' . $request->nama_pereseanuser_cari . '%');
            $query_user->orWhere('nama_lengkap', 'like', '%' . $request->nama_pereseanuser_cari . '%');
            $query_user->orWhere('level', 'like', '%' . $request->nama_pereseanuser_cari . '%');
            $query_user->orWhere('no_handphone', 'like', '%' . $request->nama_pereseanuser_cari . '%');
        }
        $query_user->orderBy("created_at", "desc");
        $pereseanuser = $query_user->paginate(7);

        $get_code = DB::select(DB::raw("SELECT MAX(right(kode_pereseanuser,2)) as kode_pereseanuser FROM tbl_user"));
        $get_code_new = $get_code[0]->kode_pereseanuser + 1;
        $uname_code_new = "PRS-NTB-0" . $get_code_new;
        //dd($uname_code_new);
        //dd($get_code);
        return view('pereseanuser.index', compact('pereseanuser', 'uname_code_new'));

    }

    public function pereseanuserstore(Request $request)
    {
        //dd($request);
        //return "pereseanuserstore";
        $uname_code_new = $request->uname_code_new;
        $peresean_namalengkap = $request->peresean_namalengkap;
        $peresean_username = $request->peresean_username;
        $peresean_nohp = $request->peresean_nohp;
        $peresean_level = $request->peresean_level;
        $peresean_password = Hash::make($request->peresean_password);

        if ($request->hasFile('peresean_foto')) {
            //dd($request->file('peresean_foto')->getClientOriginalName());
            $peresean_foto = $peresean_username . "." . $request->file('peresean_foto')->getClientOriginalExtension();
        } else {
            $peresean_foto = null;
        }
        //dd($peresean_foto);
        //dd($peresean_username);
        $cek_pereseanuser_old = DB::table('tbl_user')
            ->where('username', $peresean_username)
            ->orWhere('kode_pereseanuser', $uname_code_new)->get();

        $cek_uname_ncode = DB::table('tbl_user')
            ->where('username', $peresean_username)
            ->orWhere('kode_pereseanuser', $uname_code_new);
        //dd($cek_uname_ncode->get());
        //return $cek_uname_ncode->count();
        //dd($getcek->get());
        //$cek_pereseanuser = DB::select(DB::raw("SELECT count(*) from tbl_user where kode_pereseanuser='$uname_code_new' and username='$peresean_username'"));
        //echo sizeof($cek_pereseanuser);
        //die;
        //echo "<pre>"; print_r($cek_pereseanuser); die;
        //return sizeof($cek_pereseanuser);
        if (($cek_uname_ncode->count()) > 0) {
            return redirect()->route('datapengguna')->with(['warning' => 'Username / Kode User Sudah Ada, Silakan Cek Kembali']);
            //return $cek_uname_ncode->count();
        } else if (($cek_uname_ncode)->count() <= 0) {
            //return $cek_uname_ncode->count();
            try {
                $data_peresean = [
                    "nama_lengkap" => $peresean_namalengkap,
                    "kode_pereseanuser" => $uname_code_new,
                    "username" => $peresean_username,
                    "password" => $peresean_password,
                    "foto" => $peresean_foto,
                    "level" => $peresean_level,
                    "no_handphone" => $peresean_nohp,
                    "created_at" => date('Y-m-d H:i:s'),
                    "tgl_update" => date('Y-m-d H:i:s'),
                ];
                $simpan_peresean = DB::table('tbl_user')->insert($data_peresean);
                if ($simpan_peresean) {
                    if ($request->hasFile('peresean_foto')) {
                        $folderPath = "public/uploads/pereseanuser";
                        $request->file('peresean_foto')->storeAs($folderPath, $peresean_foto);
                    }
                    return redirect()->route('datapengguna')->with(['success' => 'Data Berhasil Disimpan']);
                } else {
                    return redirect()->route('datapengguna')->with(['warning' => 'Data Gagal Disimpan']);
                }
            } catch (\Exception $e) {
                return redirect()->route('datapengguna')->with(['warning' => 'Data Gagal Disimpan, Silakan Hubungi Admin']);
                //return redirect()->route('datapengguna')->with(['warning' => 'Username Sudah Ada, Silakan Cek Kembali']);
            }
        }
    ...
    public function pereseanuseredit(Request $request)
    {
        $id_user = $request->id_pereseanuser;
        //echo $id; die;
        $pereseanuser = DB::table('tbl_user')->where('id_user', $id_user)->first();
        //echo "<pre>"; print_r($pereseanuser);
        //die;
        return view('pereseanuser.edit', compact('pereseanuser'));
    }

    public function pereseanuserupdate($id_user, Request $request)
    {

        $uname_code_new_edit = $request->uname_code_new_edit;
        $peresean_username_edit = $request->peresean_username_edit;
        $peresean_namalengkap_edit = $request->peresean_namalengkap_edit;
        $peresean_nohp_edit = $request->peresean_nohp_edit;
        $peresean_password_edit = $request->peresean_password_edit;
        //$foto_edit = $request->foto_edit;
        $old_foto_edit = $request->old_foto_edit;
        $peresean_level_edit = $request->peresean_level_edit;

        if ($request->hasFile('foto_edit')) {
            $pereseanuser_foto = $peresean_username_edit . "." . $request->file('foto_edit')->getClientOriginalExtension();
        } else {
            $pereseanuser_foto = $old_foto_edit;
        }

        $peresean_old_pass = DB::table('tbl_user')->where('id_user', $id_user)->first()->password;
        //return $old_pass;
        if (!empty($request->peresean_password_edit)) {
            $password_tosave = Hash::make($request->peresean_password_edit);
        } else {
            $password_tosave = $peresean_old_pass;
        }

        //bisa di line ini tambah pengecekan dulu by uname / kode user sebenarnya
        try {
            $data_peresean = [
                "nama_lengkap" => $peresean_namalengkap_edit,
                "kode_pereseanuser" => $uname_code_new_edit,
                "username" => $peresean_username_edit,
                "password" => $password_tosave,
                "foto" => $pereseanuser_foto,
                "level" => $peresean_level_edit,
                "no_handphone" => $peresean_nohp_edit,
                "tgl_update" => date('Y-m-d H:i:s'),
            ];
            $update_pereseanuser = DB::table('tbl_user')->where('id_user', $id_user)->update($data_peresean);
            if ($update_pereseanuser) {
                if ($request->hasFile('foto_edit')) {
                    $folderPath = "public/uploads/pereseanuser";
                    $folderPathOld = "public/uploads/pereseanuser/" . $old_foto_edit;
                    Storage::delete($folderPathOld);
                    $request->file('foto_edit')->storeAs($folderPath, $pereseanuser_foto);
                }
                return redirect()->route('datapengguna')->with(['success' => 'Data User Berhasil di-Update']);
            } else {
                return redirect()->route('datapengguna')->with(['warning' => 'Data User Gagal di-Update']);
            }
        } catch (\Exception $e) {
            return redirect()->route('datapengguna')->with(['warning' => 'Data User Gagal di-Update, Hubungi IT']);
        }
    ...
    public function pereseanuserdelete($id_user)
    {
        $pereseanuser = DB::table('tbl_user')->where('id_user', $id_user)->first();
        $foto_to_delete = $pereseanuser->foto;
        $folderPathOld = "public/uploads/pereseanuser/" . $foto_to_delete;

        $delete = DB::table('tbl_user')->where('id_user', $id_user)->delete();
        if ($delete) {
            Storage::delete($folderPathOld);
            return redirect()->route('datapengguna')->with(['success' => 'Data Berhasil Dihapus!']);
        } else {
            return redirect()->route('datapengguna')->with(['warning' => 'Data Gagal Dihapus!']);
        }
    ...
    public function getberita($kode_satker, Request $request)
    {
        ...
        if ($roles == 'humas_kanwil') {
            ...
            return view('beritasatker.index_kanwil', compact('getmedia', 'getdivisi', 'satker', 'berita_satker', 'dtsatker'));
...
//        echo count($url_gabung);
    ...
    public function updatepassworduser($kode_pereseanuser, Request $request)
    {
        $kd_pereseanuser = $kode_pereseanuser;
        $password_lamauser = $request->password_lamauser;

        $datapereseanuser = DB::table('tbl_user')->where('kode_pereseanuser', $kd_pereseanuser)->first();
        $old_password_hashed = Hash::make($password_lamauser);

        $confirm_old_password = $request->password_lamauser; //plaintext
        $old_password = $datapereseanuser->password;

        if (Hash::check($confirm_old_password, $old_password)) {
            $password_baruuser = $request->password_baruuser;
            $konfirmasi_password_baruuser = $request->konfirmasi_password_baruuser;
            if ($password_baruuser == $konfirmasi_password_baruuser) {
                try {
                    $data_to_update = [
                        "tgl_update" => date('Y-m-d H:i:s'),
                        "password" => Hash::make($password_baruuser),
                    ];
                    $update = DB::table('tbl_user')->where('kode_pereseanuser', $kd_pereseanuser)->update($data_to_update);
                    if ($update) {
                        return redirect()->route('gantipassworduser', $kode_pereseanuser)->with(['success' => 'Data Berhasil Diupdate']);
                    } else {
                        return redirect()->route('gantipassworduser', $kode_pereseanuser)->with(['warning' => 'Data Gagal Diupdate']);

                    }
                } catch (Exception $e) {
                    return redirect()->route('gantipassworduser', $kode_pereseanuser)->with(['warning' => 'Data Gagal Diupdate, Hubungi IT']);
                }
            } else if ($password_baruuser != $konfirmasi_password_baruuser) {
                return redirect()->route('gantipassworduser', $kode_pereseanuser)->with(['warning' => 'Password Baru dan Konfirmasi Password Baru Salah']);
            }
        } else {
            return redirect()->route('gantipassworduser', $kode_pereseanuser)->with(['warning' => 'Password Lama Salah Semeton!']);
        }
...
//            $url_media_nasional_tosave = [];
    ...
    public function gantipassworduser($kode_pereseanuser, Request $request)
    {
        $kd_pereseanuser = $kode_pereseanuser;
        $pereseanuser = DB::table('tbl_user')->where('kode_pereseanuser', $kd_pereseanuser)->first();
        //dd($satker->kode_satker);
        return view('pereseanuser.updateprofile', compact('pereseanuser'));
